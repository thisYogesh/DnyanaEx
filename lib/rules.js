...
var ruleSet = function (rule, $) {
    rule.createRule(":if", function (sl, el, scope, type) {
        this.init = function () {
            this.comment = document.createComment('[:if="' + scope.replace(/this./g, '') + '"]');
        ...
        this.init = function () {
            ...
            sl.$event(el, "change", function (e, $el) {
                ...
                if (_this.isRC && _this.isRC == "radio" && $el.checked) {
                    updateObj[scope.replace('this.', '')] = $el.value;
                } else if (_this.isRC && _this.isRC == "checkbox") {
                    updateObj[scope.replace('this.', '')] = $el.checked;
                } else {
                    updateObj[scope.replace('this.', '')] = $el.value;
                }
    ...
    rule.createRule(":loop", function (sl, el, scope, type) {
        this.init = function () {
            try {
                ...
                this.bindings = [];
                this._scope = scope.replace("this.", '');
