...
var templates = function (win, $) {
    return new(extendProto(function () {
        this.root = "templates/";
        this.extention = ".html";
        this.template_tag = "template"
    }, {
        render: function (config) {
            var template = this.templates[config.template] || {},
                selector = typeof template.el == "string" ? (this.template_tag + "[name=" + template.el + "]") : template.el,
                el, $el = $(selector);
        ...
        put: function (tname, tcontent) {
            var tmp_name = tname.replace('.html', '');

            this.templates[tmp_name] = {
                content: tcontent,
                el: tmp_name
            }
        },
        ...
        get: function (templateName) {
            // if the template is not rendered the render it
            if (typeof this.templates[templateName].el == "string") {
                this.render({
