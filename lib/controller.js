...
var controller = function (w, templates, ruleEngine) {
    var $throttle_id;

    // document ready event
    var isDocReady = w.document.readyState == "complete";

    function docReady(listner) {
        setTimeout(function () {
    ...
    return extendProto(function (options) {
        ...
        setup: function () {
            extentThis(this, this.options);
            extentThis(this, this.methods);
            extentThis(this, this.data);
        },
        run: function () {
            isDocReady ? this.init() : docReady(this.init.bind(this));
        },
        init: function () {
        ...
        updateBinding: function (options) {
            ...
            for (var i = 0; i < keys.length; i++) {
                val = options[keys[i]];
                fn = Function("this." + keys[i] + ' = ' + this.makeValue(val));
                fn.apply(this);
            }
        },
